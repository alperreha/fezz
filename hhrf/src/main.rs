...
async fn handle_rpc(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        // Note: This requires the plugin to use CString::into_raw() for the response
        unsafe {
...
async fn get_or_load_library(
    ...
    // First, try to get from cache with read lock
    {
        ...
        if let Some(entry) = cache_guard.get(cache_key) {
            ...
            drop(cache_guard);
            // Update last_used timestamp on cache hit
            update_last_used(cache, cache_key).await;
            return Ok(library);
        }
    }
...
/// Updates the last_used timestamp for a cached library.
async fn update_last_used(cache: &LibraryCache, cache_key: &str) {
    let mut cache_guard = cache.write().await;
